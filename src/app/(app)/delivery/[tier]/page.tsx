...
import { getConfirmedOrdersByShippingMethod } from "@/services/orderService";
...
export default function DeliveryQueuePage() {
  ...
  React.useEffect(() => {
    const fetchQueueOrders = async () => {
      ...
      setIsLoading(true);
      setError(null);
      const { data, error: fetchError } = await getConfirmedOrdersByShippingMethod(storeId, shippingMethod);
      if (fetchError) {
